...
@Log4j
public class InsertHodActionPerformed extends BaseActionPerformedListener {

    private HeadOfDepartment headOfDepartment;

    public InsertHodActionPerformed(IOnActionPerformed onActionPerformed, HeadOfDepartment headOfDepartment) {
    ...
    public ResultMessage preActionPerformCheck() {
        ...
        if (headOfDepartment == null) {
            // fail code
            return new ResultMessage(RESULT_TYPE_FAIL, FAIL_CODE_INSERT_HEAD_OF_DEPARTMENT_INPUT_ISSUE, ResourceString.getString("fail_code_insert_hod_object_cant_null"));
        }
        ...
        if (StringUtils.isEmpty(name)) {
            // fail code
            return new ResultMessage(RESULT_TYPE_FAIL, FAIL_CODE_INSERT_HEAD_OF_DEPARTMENT_INPUT_ISSUE, ResourceString.getString("fail_code_insert_hod_name_cant_empty"));
        }
    ...
    public void actionPerform() {
        try {
            ...
            if (HeadOfDepartment.isRowExistByName(headOfDepartment.getName())) {
                // fail code
                onActionPerformed.onActionPerformedResult(new ResultMessage(RESULT_TYPE_FAIL, FAIL_CODE_INSERT_HEAD_OF_DEPARTMENT_INPUT_ISSUE, ResourceString.getString("fail_code_insert_hod_name_exist_in_db")));
                return;
            }

            // insert no issue
            //todo update the name trim value back to the object for saving purpose, otherwise it is a local copy
            HeadOfDepartment hodInsertedSuccessfully = headOfDepartment.insert();
            log.info("inserted successfully, hod: "+hodInsertedSuccessfully);
            // success code
            onActionPerformed.onActionPerformedResult(new ResultMessage(RESULT_TYPE_SUCCESS, SUCCESS_CODE_INSERT_HEAD_OF_DEPARTMENT, ResourceString.getString("hod_name_inserted_successfully")));
            return;
